...
public class Main {
    ...
    public static void PC(LinkedList<String> t) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            if (!temp.equals("X") && !temp.equals("O")) {
                t.set(i, "X");
                int puntos = algoritmoMiniMax(t, 0, false);
                t.set(i, temp);
                if (puntos > mPuntaje) {
                    mPuntaje = puntos;
                    pos = i;
    ...
    public static int algoritmoMiniMax(LinkedList<String> t, int depth, boolean tPC) {
        ...
        int mPuntaje = tPC ? Integer.MIN_VALUE : Integer.MAX_VALUE;
        for (int i = 0; i < 9; i++) {
            String temp = t.get(i);
            if (!temp.equals("X") && !temp.equals("O")) {
                t.set(i, tPC ? "X" : "O");
                int puntos = algoritmoMiniMax(t, depth + 1, !tPC);
                t.set(i, temp);
                mPuntaje = tPC ? Math.max(mPuntaje, puntos) : Math.min(mPuntaje, puntos);
            }
        }
        return mPuntaje;
    }
